...
pygame.display.set_caption("Mini Metal Slug - Dynamic Difficulty Defense Version")
...
# 적이미지
try:
    # enemy_img는 이제 일반 적과 플레이어 크기에 맞게 미리 조정합니다.
    enemy_img = pygame.image.load("allen2.png").convert_alpha()
    enemy_img = pygame.transform.scale(enemy_img, (70, 60)) # 💥 [수정] 크기를 미리 70x60으로 조정
except pygame.error:
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, current_score): 
        super().__init__()
        self.image = enemy_img # 💥 이미 70x60으로 조정된 이미지 사용

        # 💥 [수정] bottomleft를 사용하여 땅(GROUND_Y)에 정확히 맞춥니다.
        ...
        # 점수에 따른 체력 증가 로직
        level = current_score // SCORE_FOR_LEVEL_UP
        self.health = BASE_ENEMY_HEALTH + (level * HEALTH_BONUS_PER_LEVEL)
...
while True:
    ...
    # 점수 표시
    score_text = font.render(f"Score: {score} (Level: {score // SCORE_FOR_LEVEL_UP + 1})", True, WHITE)
